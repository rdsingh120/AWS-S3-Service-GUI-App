...
namespace AWSS3Service.Services
{
    public static class ObjectOperations
    {
        ...
        public static async Task UploadFile(string path, string bucketName, string fileName)
        {
            var fileTransferUtility = new TransferUtility(S3ClientProvider.s3Client);
            await fileTransferUtility.UploadAsync(path, bucketName, fileName);
        }
